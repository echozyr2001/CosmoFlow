    ...
    S: StorageBackend,
{
    ...
    pub async fn run(
    ...
    ) -> Result<Action, crate::errors::NodeError> {
        ...
        let prep_result = self
            ...
            .prep(store, &context)
            .await
            .map_err(|e| crate::errors::NodeError::PrepError(format!("Prep failed: {e}")))?;

        // Exec phase with retries
        let exec_result = self
            .exec_with_retries(prep_result.clone(), context.clone())
            .await
            .map_err(|e| crate::errors::NodeError::ExecutionError(format!("Exec failed: {e}")))?;
        ...
        let action = self
            ...
            .await
            .map_err(|e| crate::errors::NodeError::ExecutionError(format!("Post failed: {e}")))?;
