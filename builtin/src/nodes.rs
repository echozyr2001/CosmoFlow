...
use shared_store::StorageBackend;

#[cfg(feature = "storage-memory")]
use shared_store::MemoryStorage;

use crate::basic::*;

/// Convenience functions for creating nodes with MemoryStorage
/// These are only available when the storage-memory feature is enabled
#[cfg(feature = "storage-memory")]
pub mod memory {
    use super::*;
    ...
    {
        Node::new(
            ConditionalNodeBackend::new(condition, if_true, if_false).with_retries(max_retries),
        )
    }
}
