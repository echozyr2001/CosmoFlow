...
impl Node<MemoryStorage> for SimpleCounterNode {
    ...
    async fn prep(&mut self, _store: &MemoryStorage, _context: &ExecutionContext) -> Result<(), Self::Error> {
        Ok(())
    }

    async fn exec(&mut self, _prep_result: (), _context: &ExecutionContext) -> Result<(), Self::Error> {
        Ok(())
    }

    async fn post(&mut self, store: &mut MemoryStorage, _prep_result: (), _exec_result: (), _context: &ExecutionContext) -> Result<Action, Self::Error> {
        // Get current value
        let current: i32 = store.get(&self.key).map_err(|e| NodeError::from(e.to_string()))?.unwrap_or(0);
        let new_value = current + self.increment;
        
        // Store new value
        store.set(self.key.clone(), new_value).map_err(|e| NodeError::from(e.to_string()))?;
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("✅ Two-node loop completed in {} steps", result2.steps_executed);
